...
(c) 2025, Duncan Galloway duncan.galloway@monash.edu & Laurens Keek,
  laurens@xrb.space
...
__version__ = '1.35.0dev'

from .idldatabase import IDLDatabase
from .analyse import *
from .build import *
from .burstplot import *
import numpy as np
import pandas as pd
import os, re
    ...
    def get(self, field):
        ...
        if fields_ok:
            ...
            if hasattr(self, field):
                ...
                # TODO do we need a check here that the attribute is an
                # array of same length as the burst table? E.g. are there
                # no scalar attributes?? (attribute "names" has length
                # equal to the number of sources comprising the sample...)
                return getattr(self, field)[self.ind]
            else:
                logger.error('no such field or attribute: {}'.format(field))
        else:
...
class Bursts(Minbar):
    ...
    def burstplot(self, entry=None, param='flux', bdata=None, show=True,
        **kwargs):
        """
        Wrapper for burstplot, for use within the MINBAR package.
        ...
        if (bdata is None) & (entry is None) & (len(self) == 1):
            # if only one burst is selected, we can choose that
            entry = self['entry']
        elif (bdata is None) & (entry is None):
        ...
        return burstplot(bdata, param, show)
...
class Observation:
    ...
    def get_lc(self):
        ...
        def read_fits_lc(file, effarea = 1.*u.cm**2):
            ...
            timeunit = header['TIMEUNIT'].lower()
...
class Sources:
    ...
    def get(self, field, all=False):
        ...
        # data = self._f[1].data[field]
            ...
            if hasattr(self, field):
                # TODO may need to modify for consistency with Minbar.get()
                # which returns an attribute indexed by self.ind (the
		# ordering array for the current selection, which will
		# work for attributes the same length as the data table)
